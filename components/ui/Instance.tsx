...
export default function Instance({ id, imageUrl, name }: InstanceProps) {
    ...
    return (
        <div className="bg-cover bg-no-repeat bg-clip-padding overflow-clip 
            border-stroke-1 text-white hover:scale-[.98] group duration-150 rounded-in w-[512px] h-[256px] flex flex-col place-content-between"
            style={{backgroundImage: `linear-gradient(to bottom,rgba(0,0,0,0),rgba(0,0,0,0.65)), ${checkImageUrl()}`}}>
            <div className="bg-bg-1 backdrop-blur-main h-fit w-full flex gap-48 px-32 py-16 rounded-t-in opacity-0 group-hover:opacity-100 duration-150">
