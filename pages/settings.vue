    ...
    <div>
        <div class="bg-[url(/hql-bg.png)] p-[110px] gap-[110px] grid grid-cols-2 bg-cover w-screen h-screen text-white grid-fill overflow-hidden">
            ...
            <div class="flex flex-col w-full overflow-y-hidden h-full gap-[64px]">
                <div class="bg-[#0f0f0f]/60 border-2 border-white/10 flex gap-[32px] h-fit backdrop-blur-xl rounded-[10px] px-[64px] py-[8px]">
                    <a href="#" class="flex gap-[8px] place-items-center hover:scale-[.95] transition-all duration-150 active:scale-[.87]">
                        <nuxt-icon name="add" class="text-[20px]" />
                        <p class="text-white/75 text-[15px]">Add Java Directory</p>
                    </a>
                </div>
                <div class="bg-[#0f0f0f]/60 border-2 border-white/10 w-full h-fit overflow-y-auto max-[1500px]:h-fit backdrop-blur-xl rounded-[10px] p-[64px]">
                                    ...
                                    <div class="relative mt-1 z-50">
                                        ...
                                        >
                                            <ListboxOptions class="absolute bg-[#0f0f0f] mt-1 w-full overflow-auto rounded-[10px] border-2 border-white/10 py-1 text-base">
                                                <ListboxOption v-slot="{ active, selected }" v-for="option1 in launcher" :key="option1.name" :value="option1" as="template">
                                                    <li :class="[active ? 'bg-white/10 text-brand-blue-1' : 'text-white', 'relative cursor-default select-none py-2 px-4']">
                                                        <span :class="[selected ? 'font-medium' : 'font-normal', 'block truncate']">{{ option1.name }}</span>
                                                    </li>
                                                </ListboxOption>
                                            </ListboxOptions>
                                        </transition>
                            ...
                            <div class="flex place-content-between align-middle place-items-center">
                                ...
                                <Listbox v-model="selModrinth">
                                    <div class="relative mt-1 z-40">
                                        ...
                                        >
                                            <ListboxOptions class="absolute bg-[#0f0f0f] mt-1 w-full overflow-auto rounded-[10px] border-2 border-white/10 py-1 text-base">
                                                <ListboxOption v-slot="{ active, selected }" v-for="option2 in modrinth" :key="option2.name" :value="option2" as="template">
                                                    <li :class="[active ? 'bg-white/10 text-brand-blue-1' : 'text-white', 'relative cursor-default select-none py-2 px-4']">
                                                        <span :class="[selected ? 'font-medium' : 'font-normal', 'block truncate']">{{ option2.name }}</span>
                                                    </li>
                                                </ListboxOption>
                                            </ListboxOptions>
                                        </transition>
                            ...
                            <div class="flex place-content-between align-middle place-items-center">
                                ...
                                <Listbox v-model="selCurse">
                                    <div class="relative mt-1 z-30">
                                        ...
                                        >
                                            <ListboxOptions class="absolute bg-[#0f0f0f] mt-1 w-full overflow-auto rounded-[10px] border-2 border-white/10 py-1 text-base">
                                                <ListboxOption v-slot="{ active, selected }" v-for="option3 in curse" :key="option3.name" :value="option3" as="template">
                                                    <li :class="[active ? 'bg-white/10 text-brand-blue-1' : 'text-white', 'relative cursor-default select-none py-2 px-4']">
                                                        <span :class="[selected ? 'font-medium' : 'font-normal', 'block truncate']">{{ option3.name }}</span>
                                                    </li>
                                                </ListboxOption>
                                            </ListboxOptions>
                                        </transition>
                                ...
                                <div class="flex flex-col gap-[32px] h-fit">
                                    ...
                                    <Listbox v-model="selJavaInst">
                                        <div class="relative mt-1 z-50">
                                            ...
                                            >
                                                <ListboxOptions class="absolute bg-[#0f0f0f] mt-1 w-full overflow-auto rounded-[10px] border-2 border-white/10 py-1 text-base">
                                                    <ListboxOption v-slot="{ active, selected }" v-for="option4 in javaInst" :key="option4.name" :value="option4" as="template">
                                                        <li :class="[active ? 'bg-white/10 text-brand-blue-1' : 'text-white', 'relative cursor-default select-none py-2 px-4']">
                                                            <span :class="[selected ? 'font-medium' : 'font-normal', 'block truncate']">{{ option4.name }}</span>
                                                        </li>
                                                    </ListboxOption>
                                                </ListboxOptions>
                                            </transition>
                        ...
                        <div class="flex flex-col w-full gap-[32px]">
                            ...
                            <div class="flex gap-[32px]">
                                ...
                                <div class="grid grid-rows-2 grid-cols-2">
                                    <a href="#" class="text-[20px] underline hover:text-brand-blue-1"> Team </a>
                                    <a href="#" class="text-[20px] underline hover:text-brand-blue-1"> Privacy </a>
                                    <a href="https://github.com/dukcc/HQLauncher#assets" class="text-[20px] underline hover:text-brand-blue-1"> Attribution </a>
                                </div>
                            </div>
                            <button
                                @click="openModal"
    ...
    <TransitionRoot appear :show="isOpen" as="template">
        <Dialog as="div" @close="closeModal" class="relative z-10">
            <TransitionChild
                ...
                leave-to="opacity-0"
            >
                <div class="fixed inset-0 bg-black backdrop-blur-sm bg-opacity-25"></div>
            </TransitionChild>

            <div class="fixed inset-0 overflow-y-auto">
                <div class="flex min-h-full items-center justify-center p-[32px] text-center">
                    <TransitionChild
                        as="template"
                        enter="duration-300 ease-out"
                    ...
                    >
                        ...
                        >
                            <DialogTitle as="h3" class="text-[28px] font-medium leading-6 text-white"> Are you sure about this? </DialogTitle>
                            <div class="mt-2">
...
<script setup>
import { ref } from 'vue'
import { Listbox, ListboxButton, ListboxOptions, ListboxOption, TransitionRoot, TransitionChild, Dialog, DialogPanel, DialogTitle, Switch } from '@headlessui/vue'
// toggle switches
